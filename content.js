...
function buildBadgeText(range, total, remainingHours, observed) {
  ...
  return {
    ...
    text: `距下限还差 ${formatNum(needToLower, 0)} 条，距上限还有 ${formatNum(roomToUpper, 0)} 条，需 ${formatNum(rates.perHourMin, 2)}~${formatNum(rates.perHourMax, 2)}/小时 ｜ ${formatNum(rates.perDayMin, 1)}~${formatNum(rates.perDayMax, 1)}/天 ｜ 下限约 ${formatDurationHours(lowerEta)}`
  };
}
